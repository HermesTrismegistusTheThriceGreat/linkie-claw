...
[](https://res.cloudinary.com/railway/image/upload/v1645060494/docs/CLIexample_fiflvb.gif)
## Installing the CLI
...
Use the token by setting the `RAILWAY_TOKEN` environment variable and then running `railway `.
...
Use the token by setting the `RAILWAY_API_TOKEN` environment variable and then running `railway `.
...
[](https://res.cloudinary.com/railway/image/upload/v1631917786/docs/railway-link_juslvt.png)
...
`# Run  locally with the same environment variables as your Railway project
railway run cmd>`
...
- **Logged in with your Railway account** using `railway login`.
...
curl -X POST -F "[[email protected]](/cdn-cgi/l/email-protection)" https://your-app.com/admin/upload`
...
- The service must be actively running for SSH connections to work. If your service is configured with "serverless mode" and has gone to sleep, you'll need to wake it up by sending a request before attempting to SSH.
- Firewall or network restrictions: Corporate networks or restrictive firewalls may block WebSocket connections used by Railway SSH.
...
[Edit this file on GitHub](https://github.com/railwayapp/docs/edit/main/src/docs/guides/cli.md)
[
